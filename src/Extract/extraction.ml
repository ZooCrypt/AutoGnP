...
let flocal_ev ev = 
  List.fold_left (fun se (vs,_) -> 
    ...
    Vsym.S.empty ev.ev_binding 

let extract_ev file modm se =
  let ev = se.se_ev in
  let ev = 
    if ev.ev_binding = [] then 
      formula file modm None ev.ev_expr 
    else      
      ...
      let do_bd (vs,o) body = 
        ...
        Fquant_in(ev.ev_quant, (bd, ty, body), 
                  (Fv (log_oracle modm o, None))) in
      List.fold_right do_bd ev.ev_binding
        (formula ~flocal file modm None ev.ev_expr)  in
  if Game.has_assert se.se_gdef then
    f_and (Fv((modm, assertion), None)) ev
  else ev   
...
let extract_pr_se ?(local=`Local) file mem se =
  ...
  let ev = extract_ev file [modm.mod_name] se in
...
let lossless_orcl file (_,_,odecl) = 
  ...
  match odecl with
  | Odef (lc, _) -> aux 0 lc [Tac Auto]
  | Ohybrid _    -> assert false  

let pr_loosless g file form i lc = 
  let rec aux i lc tacs = 
    match lc with 
    | [] -> tacs
    ...
    | GCall(_,a,_,odef)::lc ->
      let call = 
        F.fprintf F.str_formatter "call (%s_ll (<:%s(%s).O))" 
        ...
        [Tac (Tstring ("case "^s2)) ;
          TSub [info.tacs; 
                [Tac (Tstring ("conseq * (_ : _ ==> true)"));
                 ...
                 tll;
                 TSub [ pr_loosless g1 file f_true 0 lc1; 
                        pr_loosless g2 file f_true 0 lc2]
                 ]] ] in
      let tacs = 
        [Tac (Seq(p1,p2,cut));
         TSub [ tac1; tac2]] in
...
let pr_random (pos,inv1,inv2) ju1 ju2 file =
  ...
  let lossless1 = 
    if assert2 = None then [] 
      ...
      pr_loosless g1 file (f_not assert1) 0 (c1::tl1) in
  let lossless2 = 
    if assert2 = None then [] 
    else 
      let assert2 = Fv(([g2.mod_name],assertion), None) in
      pr_loosless g2 file (f_not assert2) 0 (drop pos ju2.ju_se.se_gdef) in
...
let extract_except file pos _l pft pft' = 
  ...
  let form = extract_ev file [] ju.ju_se in
  let proof fmt () = 
    ...
    List.iter (fun (_,o) ->
      ...
      F.fprintf fmt "  let %s = g.`%s.%s in@ "
        s adv.mod_name s) ju.ju_se.se_ev.ev_binding;
...
let add_adv_add_test_bad file seoc auxname asym infob info gb = 
  ...
  let gadv_test = 
    ...
    let nc = log_oracle [] seoc.seoc_osym in
    let odef = 
        ...
        let def = 
          if Osym.equal o seoc.seoc_osym then
            ...
            Fbody {
              ...
              f_res = Some (vres, Tzc oinfo.ores);
              f_body = 
...
let add_adv_orcl_test file g evars etuple ctests t1 t2 seoc = 
  ...
  let cv = log_oracle [] seoc.seoc_osym in
  let gvar = 
  ...
  let a1,a2 = 
    ...
    let c1, c2 = 
      ...
      aux [] body.f_body in
    let c1 = 
      let vc = log_oracle [] seoc.seoc_osym in
...
let proof_OrclTestM file osym ju gadv tOT tests =
  ...
  let rec gaux lc info =
    ...
    | GCall(vs,_,_,odef)::lc -> gaux lc (add_call vs odef info) 
    | GAssert _ :: _ -> assert false (* FIXME *) in
  
  let vcs = log_oracles file in
  let nvc = List.length vcs in
  let vco = log_oracle [] osym in
  let eqvc =
    let mk_eq v = f_eq (f_v g v "1") (f_v gadv v "2") in
    List.fold_left (fun inv (v,_) ->
      let eq = 
...
let proof_OrclB file infob tOT advOT mb aAUX jucb seoc etuple et2 = 
  ...
  let vcs = log_oracles_i infob in
  let nvc = List.length vcs in
  let vco = log_oracle [mb] seoc.seoc_osym in
  let eqvcw0 =
    let mk_eq v = f_eq (Fv(([advOT],v),Some "1")) (Fv(([mb],v),Some "2")) in
    List.fold_left (fun inv ((_,v),_) ->
  ...
  let eqi = 
    let c = snd (log_oracle [] seoc.seoc_osym) in
    f_and 
  ...
  let len1 = len + List.length (log_oracles file) - 1 in
...
let rec extract_proof file pft = 
  ...
  | Rassert(p,e) -> 
    ...
    extract_proof_trans "assert" file lemma1 cmp1 bound1 pft pft2
