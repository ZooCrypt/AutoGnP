...
let mpoly_div_f  vars p q =
  let p = poly_to_ipoly vars p and q = poly_to_ipoly vars q in
  let res = Factory.div p q in
  ipoly_to_poly vars res;;
...
let eps_to_fracs (eps:(NormField.EP.t * Expr.expr) list ) =
  ...
  map (fun (p,i) -> (p, unit vars)) polys,vars,mp

let fracs_to_eps fracs vars mp =
  let frac1,frac2 = List.split fracs in
  let frac1 = polys_to_eps (map (fun p-> (p,[])) frac1) vars mp |> map (fun (p,i)->p) in
  let frac2 = polys_to_eps (map (fun p-> (p,[])) frac2) vars mp |> map (fun (p,i)->p) in
...
let rec fgrobner vars mp (basis:basis_r) pairs =
  (*print_string(string_of_int(length basis)^" basis elements and "^
                string_of_int(length pairs)^" pairs");
    print_newline();*)
  ...
  | (p1,p2)::opairs ->
    try
      ...
      if fst (fst sp) = [] then fgrobner vars mp basis opairs
      (*else if for_all (for_all ()) sp then basis*) else
        (let [(debug)] =  (fracs_to_eps [fst sp] vars mp)  in
         ...
         fgrobner vars mp (new_gen::basis) (opairs @ newcps)
        )
    with Failure "mdiv" -> fgrobner vars mp basis opairs;;

(* ------------------------------------------------------------------------- *)
(* Overall function.                                                         *)
(* ------------------------------------------------------------------------- *)

let fgroebner vars mp basis = fgrobner vars mp basis (distinctpairs basis);;
...
let fintersect vars mp (gen1:basis_r) (gen2:basis_r) :basis_r =
  ...
  let basis = fgroebner vars mp ((tcombine (List.map (fun (f,i)-> frac_mul vars t_poly f, frac_mul vars t_poly i) fracs) pvars da)@(tcombine (List.map (fun (f,i)-> frac_mul vars mt_poly f, frac_mul vars t_poly i) fracs2) pvars2 da2)) in
...
let rnd_deduce (vars:int list) mp rndvars pvars fracs ((pol,q):frac) :basis_r=
  ...
  let debugs =  (fracs_to_eps fracs vars mp)  in
   iter   (fun debug->    log_i (lazy (fsprintf "Gen_l %a " pp_expr debug  ))) debugs;
  let debugs =  (polys_to_eps (map (fun p-> (p,0)) splits) vars mp)  in
   iter   (fun debug->    log_i (lazy (fsprintf "splits %a " pp_expr debug  ))) (map (fun (p,q)->p)debugs);
...
let get_rnd vars mp gens pol =
  let red,rnds =  freduce vars mp gens pol in
                       ((fst red)= [],rnds);; 
