  ...
  LP.wrapError (fun _ ->
    ...
    let ensure_fresh rn ju =
      if Se.exists (fun e -> name (destr_R e).Rsym.id = rn) (rvars_cj ju) then
        raise (LP.CpaWrapError("cmd", Printf.sprintf "random variable %s not fresh" rn))
    in
    let handleCmd c ps =
      ...
      | PU.COpt(rn,ie) ->
          let r = LP.getRsym te rn in
          let e = LP.convert_expr te ie in
          if (Se.mem (mk_R r) (e_rvars e)) 
          then raise (LP.CpaWrapError("cmd", "opt (r) (e): random variable r occurs in expression e"));
          { ps with ps_proof = app (appOpt r e) prf }
      | PU.CPerm(rn,fn) ->
          let r = LP.getRsym te rn in
          let f = LP.getPsym te fn in
          { ps with ps_proof = app (appPerm r f) prf }
      | PU.CSplit(r,it1,r1n,r2n) ->
          (let r = LP.getRsym te r in 
           let ty1 = LP.getTy te it1 in
           ensure_fresh r1n ju;
           ...
           | _ -> raise (LP.CpaWrapError("cmd", Printf.sprintf "invalid type for %s" r1n)))
      | PU.CMerge(r1n,r2n,rn) ->
          let r1 = LP.getRsym te r1n in
          let r2 = LP.getRsym te r2n in
          let tyr = ty_concat r1.Rsym.ty r2.Rsym.ty in
          ensure_fresh rn ju;
          let r = Rsym.mk rn tyr in
          { ps with ps_proof = app (appMerge r1 r2 r) prf }
      | PU.CFail1(hn,rn) ->
          let h = LP.getHsym te hn in
      ...
      | PU.CRnd(rn, ic) ->
          ...
          let c = LP.convert_expr ~star:ty te ic in
          { ps with ps_proof = app (appRnd r c) prf }
      | PU.COw(fn,r1n,t1n,t2n,r2ns,ic1,ic2) ->
          let r2vs = List.map (LP.getRsym te) r2ns in
          let r1v = LP.getRsym te r1n in
          let qes = askEventMessages ju.cj_ev in
          ...
          let c1 = LP.convert_expr ~star:star1_ty te ic1 in
          let star2_ty = ty_concat_l [r1v.Rsym.ty; r2s_ty; msg_ty] in
          let c2 = LP.convert_expr ~star:star2_ty te ic2 in
          let t1 = LP.getTy te t1n in
          let t2 = LP.getTy te t2n in
          let f  = LP.getPsym te fn in
...
let norm_for_path path ps se =
  LP.wrapError (fun _ ->
    ...
    let e = LP.convert_expr ju ie in
    Norm.norm e)
...
let apply_ctxt_for_path path ps sknown sctxt =
  LP.wrapError (fun _ ->
    ...
    let known = LP.convert_expr ju iknown in
    let ictxt = Parse.expr sctxt in
    let ctxt = LP.convert_expr ~star:known.e_ty ju ictxt in
...
let ps_of_tps tps : proofState =
  ...
  let cp = LP.declare_scheme ts.ts_msg_decl ts.ts_hash_decls ts.ts_perm_decls ts.ts_rvar_decls ts.ts_cipher in
