...
let norm_ggt e =   
  ...
  | G -> mk_gexp (mk_GLog x)   (* g ^ (log x) *)
  | GT -> mk_gtexp (mk_GTLog x) (* e(g,g) (tlog x) *)
  | Fq | Bool | BS _ -> e
  | Prod _ -> e
...
let rec mk_simpl_op op l =
  ...
  | GExp, [g1;p1] ->
    ...
    let p = norm_expr_field (mk_FMult a p1) in
    mk_gexp p
  ...
  | EMap, [g1;g2] -> (* e(g^a,g^b) -> e(g,g)^ab *)
    ...
    let p = norm_expr_field (mk_FMult p1 p2) in
    mk_gtexp p
...
and mk_simpl_nop op l =
  ...
  match op with
  | GMult -> 
    let lp = List.map destr_gexp l in
    let p = norm_expr_field (mk_FPlus lp) in
    mk_gexp p
  | GTMult ->
    let lp = List.map destr_gtexp l in
    let p = norm_expr_field (mk_FPlus lp) in
    mk_gtexp p
  | FPlus  | FMult  ->
    assert false (* norm_expr_field should be called instead *)
  ...
  | Land -> 
    ...
    let rec aux l = 
      ...
      | [e] -> 
        if is_True e then [] 
        else is_False e then raise Not_found
        else l
      | e1::((e2::l) as l1) -> 
        if is_False e1 then raise Not_found 
        else 
          if e_equal e1 e2 || is_True e1 then aux l1
          else e1 :: aux l1 in
    try 
      let l = aux l in
      if l = [] then mk_True 
...
and norm_expr e = 
  ...
  | Proj(i,e) -> mk_simpl_proj i (norm_expr e)
  | ElemH(e,h) ->
